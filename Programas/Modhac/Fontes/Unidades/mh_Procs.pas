...
Function LeArqChuvas (Const Nome: String;
                      Carac_Fim_Reg: Char;
...
Begin
  ...
  Try
    Reset(Arquivo);
    Result := TwsDataset.Create('Chuvas');

    // Assinatura - Versão do Arquivo
    Result.ExtraData[0] := 1;
    Result.ExtraData[1] := 5;
    Result.ExtraData[2] := 0;
...
begin
  ...
  While Not EOF(Arquivo) Do
    Begin
    inc(k);
    if (k mod 1000) = 0 then Application.ProcessMessages();
    Read(Arquivo, Ch);
    if Ch = '-' then
       begin
       valor := '-';

...
var
  ...
  begin
    Result := 'Os arquivos seguem o padrão de exportação feitos pelo site HidroWEB'#13#10 +
              'Formato do Arquivo: Texto com 75 colunas separadas por ";"'#13#10 +
              'Separador Decimal: vírgula' + #13#10 +
              '  - 1.Coluna: Identificação do Posto'#13#10 +
              '  - 3.Coluna: Data'#13#10 +
              '  - 14. 15. ... 44.Coluna: Valores (Um para cada dias do mes)'#13#10 +
              '  - 45. 46. ... 75.Coluna: Status do Dado (Um para cada dias do mes)';
  end;
  ...
  begin
    ...
    for i := 0 to SL.Count-1 do
      if System.Pos('//EstacaoCodigo;NivelConsistencia;', SL[i]) > 0 then
         begin
         ii := i + 1;
         break;
         end;
    ...
    try
      for i := ii to SL.Count-1 do
        ...
        if Dados.Count <> 76 then Continue;

        // Informações básicas
        Posto := Dados[0];
        Data := StrToDate(Dados[2]);
        DecodeDate(Data,Ano,Mes,Dia);
        Dias := DaysInAMonth(Ano,Mes);
        ...
        for ii := 13 to 44 do
          ...
          if Dia <= Dias then
             ...
             k := ii + 31;
             s := Dados[ii];
             if (s = '') then Dado := wscMissValue else Dado := StrToFloat(s);

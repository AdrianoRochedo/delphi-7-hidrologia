...
type
  ...
  public
    ...
    // Ocorre quando ha uma mudanca nos itens selecionados
...
begin
  ...
  for i := 0 to clPostos.Items.Count-1 do
    if clPostos.checked[i] then
       FPostosSel.AddObject(clPostos.Items[i], pointer(i + cPostos));
...
procedure TFrame_SelecaoDePostos.DoCheckChange();
begin
  if Assigned(FOnCheckChange) then
     FOnCheckChange(clPostos);
end;

...
type
  ...
  TProc_setProcedaControlInterface = procedure (const intf: IProcedaControl);
  TProc_setProcessControlInterface = procedure (const intf: IProcessControl);
  TProc_setDataControlInterface    = procedure (const intf: IDataControl);
  TProc_Import                     = function  (const Files: TStrings): boolean;
  TProc_Export                     = function: boolean;
  ...
  public
    ...
    setProcedaControlInterface : TProc_setProcedaControlInterface;
  ...
  TExportPlugin = class (TCommomPlugin)
  public
    Export : TProc_Export;
    setDataControlInterface : TProc_setDataControlInterface;
  ...
  if h <> 0 then
     try
       ...
       if Assigned(proc) then
          ...
          if proc() = cDataExport then
             p := TExportPlugin.Create()
          else
             begin
             p := nil;
             Exit;
...
begin
  ...
  if Handle <> 0 then
     ...
     setProcedaControlInterface :=
       GetProcAddress(Handle, 'setProcedaControlInterface');
...
constructor TExportPlugin.Create;
begin
  inherited Create();
  FPT := ptDataExport;
end;

procedure TExportPlugin.LoadProcs(h: THandle);
begin
  inherited LoadProcs (h);
  Export := GetProcAddress(Handle, 'Export');
  setDataControlInterface :=
    GetProcAddress(Handle, 'setDataControlInterface');
end;

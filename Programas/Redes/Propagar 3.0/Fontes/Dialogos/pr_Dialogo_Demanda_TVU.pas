...
uses
  ...
  StdCtrls, Buttons, gridx32, drEdit, pr_Const, pr_Tipos, DiretivasDeCompilacao;
...
begin
  if Tab.Cells[0, 1] = '' then
     ...
     for i := 1 to Tab.RowCount-1 do
       begin
       if SubStrings('-', s1, s2, Tab.Cells[0, i]) > 0 then
          begin
          Dados[i-1].AnoIni := strToInt(AllTrim(s1));
          Dados[i-1].AnoFim := strToInt(AllTrim(s2));
          end
       else
          begin
...
begin
  ...
  for k := 1 to Tab.RowCount-1 do
    begin
    SubStrings('-', s1, s2, Tab.Cells[0, k]);
    i := strToInt(AllTrim(s1));
    ii := strToInt(AllTrim(s2));
...
begin
  if (Tipo = tdDemanda) and (Tab.Row > 0) and (Tab.Col > 0) then
     begin
     SubStrings('-', s1, s2, Tab.Cells[0, Tab.Row]);
     AnoInicial.AsString := AllTrim(s1);
     AnoFinal.AsString   := AllTrim(s2);
     end;
end;

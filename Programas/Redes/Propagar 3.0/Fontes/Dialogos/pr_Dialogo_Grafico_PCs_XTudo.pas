...
uses
  ...
  StdCtrls, Buttons, ExtCtrls, Form_Chart,
  pr_Classes,
  esquemas_OpcoesGraf;
...
begin
  ...
  if cbLinhas.Checked then
     begin
     g1 := PC.CriarGrafico_Default(PC.Nome + ' Series', k);
     ...
     if cbVT.Checked Then
        ...
        if (Fim <= PC.AfluenciaSB.Len) and (Fim <= PC.VazMontante.Len) then
           ...
           b := g1.Series.AddLineSerie('Vazão Total', clNavy, V, Ini,Fim);
     ...
     if cbVSB.Checked then
        begin
        b := g1.Series.AddLineSerie('Vazões das Sub-Bacias', clBlack, PC.AfluenciaSB, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbVM.Checked then
        begin
        b := g1.Series.AddLineSerie('Vazões de Montante', clGray, PC.VazMontante, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDef.Checked then
        begin
        b := g1.Series.AddLineSerie('Defluência', clTeal, PC.Defluencia, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     ...
     if cbDATot.Checked Then
        ...
        if (Fim <= PC.DemPriAtendida.Len) and (Fim <= PC.DemSecAtendida.Len) and (Fim <= PC.DemTerAtendida.Len) then
           ...
           b := g1.Series.AddLineSerie('Dem. Tot. Atendida', clMaroon, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbDAP.Checked then
        begin
        b := g1.Series.AddLineSerie('Demanda Primária Atendida', clGreen, PC.DemPriAtendida, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
     ...
     if cbDAS.Checked then
        begin
        b := g1.Series.AddLineSerie('Demanda Secundária Atendida', clYellow, PC.DemSecAtendida, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
     ...
     if cbDAT.Checked then
        begin
        b := g1.Series.AddLineSerie('Demanda Terciária Atendida', clBlue, PC.DemTerAtendida, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
     ...
     if cbDPTot.Checked Then
        ...
        if (Fim <= PC.DemPriPlanejada.Len) and (Fim <= PC.DemSecPlanejada.Len) and (Fim <= PC.DemTerPlanejada.Len) then
           ...
           b := g1.Series.AddLineSerie('Dem. Tot. Planejada', clBlack, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbDPP.Checked then
        begin
        b := g1.Series.AddLineSerie('Demanda Primária Planejada', clPurple, PC.DemPriPlanejada, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDPS.Checked then
        begin
        b := g1.Series.AddLineSerie('Demanda Secundária Planejada', clNavy, PC.DemSecPlanejada, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDPT.Checked then
        begin
        b := g1.Series.AddLineSerie('Demanda Terciária Planejada', clLime, PC.DemTerPlanejada, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     ...
     if cbDRTot.Checked Then
        ...
        if (Fim <= PC.DemPriTotal.Len) and (Fim <= PC.DemSecTotal.Len) and (Fim <= PC.DemTerTotal.Len) then
           ...
           b := g1.Series.AddLineSerie('Dem. Tot. Ref ', clYellow, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbDRP.Checked then
        begin
        b := g1.Series.AddLineSerie('Dem. Pri. Ref', clLime, PC.DemPriTotal, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDRS.Checked then
        begin
        b := g1.Series.AddLineSerie('Dem. Sec. Ref', clNavy, PC.DemSecTotal, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDRT.Checked then
        begin
        b := g1.Series.AddLineSerie('Dem. Ter. Ref', clAqua, PC.DemTerTotal, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbEG.Checked then
        begin
        b := g1.Series.AddLineSerie('Energia', $00F1BECF, TprPCP(PC).Energia, Ini, Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     end;
  ...
  if cbBarras.Checked then
     begin
     g2 := PC.CriarGrafico_Default('Séries do ' + PC.Nome, k);
     ...
     if cbVT.Checked Then
        ...
        if (Fim <= PC.AfluenciaSB.Len) and (Fim <= PC.VazMontante.Len) then
           ...
           b := g2.Series.AddBarSerie('Vazão Total', clNavy, 0, 1, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbVSB.Checked then
        begin
        b := g2.Series.AddBarSerie('Vazões das Sub-Bacias', clBlack, 0, 1, PC.AfluenciaSB, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbVM.Checked then
        begin
        b := g2.Series.AddBarSerie('Vazões de Montante', clGray, 0, 1, PC.VazMontante, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDef.Checked then
        begin
        b := g2.Series.AddBarSerie('Defluência', clTeal, 0, 1, PC.Defluencia, Ini, Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     ...
     if cbDATot.Checked Then
        ...
        if (Fim <= PC.DemPriAtendida.Len) and (Fim <= PC.DemSecAtendida.Len) and (Fim <= PC.DemTerAtendida.Len) then
           ...
           b := g2.Series.AddBarSerie('Dem. Tot. Atendida', clMaroon, 0, 1, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbDAP.Checked then
        begin
        b := g2.Series.AddBarSerie('Dem. Pri. Atendida', clGreen, 0, 1, PC.DemPriAtendida, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        if cbFalhas.Checked then DestacaFalhas(1);
        end;
     ...
     if cbDAS.Checked then
        begin
        b := g2.Series.AddBarSerie('Dem. Sec. Atendida', clYellow, 0, 1, PC.DemSecAtendida, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        if cbFalhas.Checked then DestacaFalhas(2);
        end;
     ...
     if cbDAT.Checked then
        begin
        b := g2.Series.AddBarSerie('Dem. Ter. Atendida', clBlue, 0, 1, PC.DemTerAtendida, Ini, Fim);
        PC.DefinirEixoX_Default(b, k);
        if cbFalhas.Checked then DestacaFalhas(3);
        end;
     ...
     if cbDPTot.Checked Then
        ...
        if (Fim <= PC.DemPriPlanejada.Len) and (Fim <= PC.DemSecPlanejada.Len) and (Fim <= PC.DemTerPlanejada.Len) then
           ...
           b := g2.Series.AddBarSerie('Dem. Tot. Planejada', clBlack, 0, 1, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbDPP.Checked then
        begin
        b := g2.Series.AddBarSerie('Dem. Pri. Planejada', clMaroon, 0, 1, PC.DemPriPlanejada, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDPS.Checked then
        begin
        b := g2.Series.AddBarSerie('Dem. Sec. Planejada', clGreen, 0, 1, PC.DemSecPlanejada, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDPT.Checked then
        begin
        b := g2.Series.AddBarSerie('Dem. Ter. Planejada', clLime, 0, 1, PC.DemTerPlanejada, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     ...
     if cbDRTot.Checked Then
        ...
        if (Fim <= PC.DemPriTotal.Len) and (Fim <= PC.DemSecTotal.Len) and (Fim <= PC.DemTerTotal.Len) then
           ...
           b := g2.Series.AddBarSerie('Dem. Tot. Ref', clYellow, 0, 1, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbDRP.Checked then
        begin
        b := g2.Series.AddBarSerie('Dem. Pri. Ref', clLime, 0, 1, PC.DemPriTotal, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDRS.Checked then
        begin
        b := g2.Series.AddBarSerie('Dem. Sec. Ref', clNavy, 0, 1, PC.DemSecTotal, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDRT.Checked then
        begin
        b := g2.Series.AddBarSerie('Dem. Ter. Ref', clAqua, 0, 1, PC.DemTerTotal, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbEG.Checked then
        begin
        b := g2.Series.AddBarSerie('Energia', $00F1BECF, 0, 1, TprPCP(PC).Energia, Ini, Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     end;
  ...
  if cbPontos.Checked then
     begin
     g1 := PC.CriarGrafico_Default('Séries do ' + PC.Nome, k);
     ...
     if cbVT.Checked Then
        ...
        if (Fim <= PC.AfluenciaSB.Len) and (Fim <= PC.VazMontante.Len) then
           begin
           for i := Ini to Fim do V[i] := PC.AfluenciaSB[i] + PC.VazMontante[i];
           b := g1.Series.AddPointSerie('Vazão Total', clNavy, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbVSB.Checked then
        begin
        b := g1.Series.AddPointSerie('Vazões das Sub-Bacias', clBlack, PC.AfluenciaSB, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbVM.Checked then
        begin
        b := g1.Series.AddPointSerie('Vazões de Montante', clGray, PC.VazMontante, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDef.Checked then
        begin
        b := g1.Series.AddPointSerie('Defluência', clTeal, PC.Defluencia, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     ...
     if cbDATot.Checked Then
        ...
        if (Fim <= PC.DemPriAtendida.Len) and (Fim <= PC.DemSecAtendida.Len) and (Fim <= PC.DemTerAtendida.Len) then
           ...
           b := g1.Series.AddPointSerie('Dem. Tot. Atendida', clMaroon, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbDAP.Checked then
        begin
        b := g1.Series.AddPointSerie('Dem. Pri. Atendida', clGreen, PC.DemPriAtendida, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        if cbFalhas.Checked then DestacaFalhas(1);
        end;
     ...
     if cbDAS.Checked then
        begin
        b := g1.Series.AddPointSerie('Dem. Sec. Atendida', clYellow, PC.DemSecAtendida, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        if cbFalhas.Checked then DestacaFalhas(2);
        end;
     ...
     if cbDAT.Checked then
        begin
        b := g1.Series.AddPointSerie('Dem. Ter. Atendida', clBlue, PC.DemTerAtendida, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        if cbFalhas.Checked then DestacaFalhas(3);
        end;
     ...
     if cbDPTot.Checked Then
        ...
        if (Fim <= PC.DemPriPlanejada.Len) and (Fim <= PC.DemSecPlanejada.Len) and (Fim <= PC.DemTerPlanejada.Len) then
           ...
           b := g1.Series.AddPointSerie('Dem. Tot. Planejada', clBlack, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbDPP.Checked then
        begin
        b := g1.Series.AddPointSerie('Dem. Pri. Planejada', clPurple, PC.DemPriPlanejada, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDPS.Checked then
        begin
        b := g1.Series.AddPointSerie('Dem. Sec. Planejada', clNavy, PC.DemSecPlanejada, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDPT.Checked then
        begin
        b := g1.Series.AddPointSerie('Dem. Ter. Planejada', clLime, PC.DemTerPlanejada, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     ...
     if cbDRTot.Checked Then
        ...
        if (Fim <= PC.DemPriTotal.Len) and (Fim <= PC.DemSecTotal.Len) and (Fim <= PC.DemTerTotal.Len) then
           ...
           b := g1.Series.AddPointSerie('Dem. Tot. Ref ', clYellow, V, Ini,Fim);
           PC.DefinirEixoX_Default(b, k);
           end;
     ...
     if cbDRP.Checked then
        begin
        b := g1.Series.AddPointSerie('Dem. Pri. Ref', clLime, PC.DemPriTotal, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDRS.Checked then
        begin
        b := g1.Series.AddPointSerie('Dem. Sec. Ref', clNavy, PC.DemSecTotal, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbDRT.Checked then
        begin
        b := g1.Series.AddPointSerie('Dem. Ter. Ref', clAqua, PC.DemTerTotal, Ini,Fim);
        PC.DefinirEixoX_Default(b, k);
        end;

     if cbEG.Checked then
        begin
        b := g1.Series.AddPointSerie('Energia', $00F1BECF, TprPCP(PC).Energia, Ini, Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     end;

...
uses                                          
  ...
  pr_Classes, StdCtrls, AxCtrls, OleCtrls, vcf1, Menus, pr_Tipos;
...
type
  ...
  private
    FFalhas: TprListaDeFalhas;
    procedure SetFalhas(const Value: TprListaDeFalhas);
  public
    property Falhas: TprListaDeFalhas read FFalhas write SetFalhas;
  end;
...
procedure TprDialogo_FalhasDeUmPC.SetFalhas(const Value: TprListaDeFalhas);
var i: Integer;
begin
  FFalhas := Value;
...
begin
  ...
  for i := 0 to F.Intervalos.Count-1 do
    ...
    d := FFalhas.Projeto.IntervaloParaData(F.Intervalos[i]);

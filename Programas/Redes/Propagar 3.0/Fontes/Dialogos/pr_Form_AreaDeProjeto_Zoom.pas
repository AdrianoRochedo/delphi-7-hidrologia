...
// virtual
function TfoAreaDeProjeto_Zoom.CriarProjeto(TN: TStrings): TprProjetoOtimizavel;
begin
...
(*
  if (Projeto.ArqFundo <> '') and MostrarFundo and (ZoomPanel.Bitmap = nil) then
     ...
     ZoomPanel.SetSecondScalePoint(
       MakePoint(ZoomPanel.Bitmap.Width, ZoomPanel.Bitmap.Height),
...
procedure TfoAreaDeProjeto_Zoom.btnAbrirClick(Sender: TObject);
begin
  Open.InitialDir := Applic.LastDir;
  if Open.Execute() then
     begin
     ZoomPanel.ClearBitmap();
     Projeto.LoadFromXML(Open.FileName);
     Applic.LastDir := ExtractFilePath(Open.FileName);
     Fit();
     end
end;

procedure TfoAreaDeProjeto_Zoom.sbFitClick(Sender: TObject);
begin
  Fit();
end;

procedure TfoAreaDeProjeto_Zoom.Fit();
begin
  ZoomPanel.Fit();
end;

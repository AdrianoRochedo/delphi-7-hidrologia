...
// FileVersion 5 : Acrescimo da propriedade "Projeto.PermitirVazoesNegativasNasSubBacias"
...
begin
  ...
  CreateForm(TMainForm, Propagar.MainForm.MainForm);
  Main := Propagar.MainForm.MainForm;

  Main.Caption := ' ' + Forms.Application.Title + ' ' + Version;

  if FPersonalReg <> '' then
     Main.Caption := Main.Caption + ' - Registrado para ' + FPersonalReg;

  if dir_NivelDeRestricao <> 0 then
     Main.Caption := Main.Caption + ' - Nível de Restrição ' + toString(dir_NivelDeRestricao);

  {$ifdef VERSAO_LIMITADA}
  Main.Caption := Main.Caption + ' (VERSÃO LIMITADA)';
  {$endif}
end;
...
begin
  ...
  ini.WriteInteger('Principal', 'x' , Propagar.MainForm.MainForm.Left);
  ini.WriteInteger('Principal', 'y' , Propagar.MainForm.MainForm.Top);
  ini.WriteInteger('Principal', 'dx', Propagar.MainForm.MainForm.Width);
  ini.WriteInteger('Principal', 'dy', Propagar.MainForm.MainForm.Height);
  ini.WriteInteger('Principal', 'ws', ord(Propagar.MainForm.MainForm.WindowState));
  ...
  for i := 1 to System.ParamCount do
    if SysUtils.CompareText(ParamStr(i), 'SEMSPLASH') = 0 then
       ...
       if System.pos('\', System.ParamStr(i)) > 0 then
          Propagar.MainForm.MainForm.CreateMDIChild(System.ParamStr(i), False, False, -1);
...
var s: string;
begin
  if ActiveObject is THidroComponente then
     s := Format('Objeto: %s'#13 +
                 'Erro: %s',
                 [
                   THidroComponente(ActiveObject).Nome,
                   msg
                 ])
  else
     s := 'Erro: ' + msg;
...
begin
  Propagar.MainForm.MainForm.CreateMDIChild(Filename, false, false, -1);
end;

...
type
  ...
  private
    ...
    procedure MenuPlotarCropYield(Sender: TObject);
 ...
 public
    ...
    // Moeda utilizada
    property MonetaryUnit : string read FMonetaryUnit write FMonetaryUnit;
...
begin
  ...
  FDescription := TXML_StringList.Create();
...
begin
  x.beginTag('management');
    ...
    FDescription.ToXML('Description', x.Buffer, x.IdentSize);
...
begin
  ...
  if FCrop.FxmlVersion >= 13 then
     FDescription.FromXML(cn.nextNode());
...
begin
  ...
  CreateAction(Actions, nil, 'Edit Description ...', false, EditEvent, nil);
...
var p: TSpreadSheetBook;
    ...
    y: TYear;
begin
  p := TSpreadSheetBook.Create('', 'Common Data');
  FCrop.AddResult(p);
...
procedure TManagement.EditEvent(Sender: TObject);
begin
  DialogUtils.EditText(FDescription);
end;

function TManagement.getDescription(): string;
var i: Integer;
begin
  result := '';

  if not self.FIrrigate then
     result := '<font color="red"><b>Rainfed</b></font><br><br>';

  for i := 0 to FDescription.Count-1 do
    result := result + FDescription[i] + '<br>';
end;
...
begin
  if CompareText(Propriedade, 'MonetaryUnit') = 0 then
     result := self.FMonetaryUnit
  else
     Result := '';
end;
...
begin
  ...
  // Outros Dados
  x.Write('MonetaryUnit', FMonetaryUnit);
...
begin
  ...
  if FxmlVersion >= 13 then
     begin
     FMonetaryUnit := cn.nextNode().text;
     end;
     ...
     Act1 := CreateAction(Actions, nil, 'Analysis', false, nil, nil);
       Act2 := CreateAction(Actions, Act1, 'Graphics', false, nil, nil);
          ...
          Act3 := CreateAction(Actions, Act2, 'Yields', false, nil, nil);
             ...
             CreateAction(Actions, Act3, 'Crop Yield  ...', false, MenuPlotarCropYield, nil);
...
procedure TCrop.MenuPlotarCropYield(Sender: TObject);
begin
  FManagements.Plot(tiCropYield);
end;

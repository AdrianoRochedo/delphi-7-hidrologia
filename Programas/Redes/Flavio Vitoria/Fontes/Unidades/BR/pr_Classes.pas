...
type
  ...
  public
    ...
    property  Figura      : TdrBaseShape    read FImagemDoComponente;
  end;
  ...
  public
    ...
    procedure GerarEquacoes(Text: TStrings; DeltaT: integer; var IndiceGeral: integer);
  ...
  private
    ...
    function ObterVazaoAfluente(PC: TprPC): Real;
  ...
  protected
    ...
    //procedure Evento_BlocoEquacao (Sender: TObject; const EventID: Integer);
  public
    ...
    procedure Equacoes_GerarBalancoHidrico(DeltaT: integer);
...
{
    property Equacoes_IntervaloInicial : Integer read  FEquacoes_IntervaloInicial
                                                 write FEquacoes_IntervaloInicial;

    property Equacoes_IntervaloFinal : Integer read  FEquacoes_IntervaloFinal
                                               write FEquacoes_IntervaloFinal;

    property Equacoes_ArquivoDeInicializacao : string read  FEquacoes_ArquivoDeInicializacao
                                                      write FEquacoes_ArquivoDeInicializacao;

    property Equacoes_NomeScriptComplementar : string read  FEquacoes_NomeScriptComplementar
                                                      write FEquacoes_NomeScriptComplementar;
}
...
begin
  ...
  for i := 0 to FProjeto.Intervalos.IntFim[Intervalo] - k do
    if i < Serie.Labels.Count then
       Serie.XLabel[i] := IntervaloToStrData(FProjeto, k + i) +
                          ' (' + ToString(k + i) + ')';
end;
...
function TprPC.ObterVazaoAfluenteSBs: Real;
var i: Integer;
begin
  // SubBacia.ObterVazaoAfluente depende do deltaT atual do projeto
  Result := 0;
  for i := 0 to SubBacias-1 do
    Result := Result + SubBacia[i].ObterVazaoAfluente(Self);
end;
...
// Palavras-Chaves: Equacao, Equacoes, Equation, Equations
procedure TprPC.GerarEquacoes(Text: TStrings; DeltaT: integer; var IndiceGeral: integer);
  ...
  var i: integer;
      s: string;
  begin
    s := 'QM' + FNome + toString(DeltaT) + ' - ';

    for i := 0 to PCs_aMontante-1 do
      s := s + 'QJ' + PC_aMontante[i].Nome + toString(DeltaT) + ' - ';

    for i := 0 to PCs_aMontante-1 do
      begin
      s := s + 'RT' + PC_aMontante[i].Nome + toString(DeltaT);
...
begin
  if FHierarquia > 1 then
     ...
     Add('QM' + FNome + toString(DeltaT) + ' = 0.0');

  FEQL.ToStrings(Text, DeltaT, IndiceGeral);
  Text.Add('');
end;
...
begin
  ...
  if FieldName = 'QM' then
     Result := FloatToStr( ObterVazoesDeMontante() )
  else
  if FieldName = 'SB' then
     Result := FloatToStr( ObterVazaoAfluenteSBs() )
  else
  if FieldName = 'DM' then
     Result := 'DM_NAO_DEF'
  else
  if FieldName = 'QJ' then
     Result := FloatToStr( Defluencia[FProjeto.DeltaT] )
  else
...
begin
  ...
  FAfluenciaSB[i] := ObterVazaoAfluenteSBs;

  // Cálculo do campo VazõesDeMontante
  FVzMon[i] := ObterVazoesDeMontante;
...
begin
  ...
  for i := 1 to FAfluenciaSB.Len do
    ...
    for j := 0 to SubBacias-1 do
      FAfluenciaSB[i] := FAfluenciaSB[i] + SubBacia[j].ObterVazaoAfluente(Self);
...
begin
  ...
  // Equacao 0:  QM - DM - QJ = - QSB
  e := TEquation.Create();
  e.Add(TIdentificator.Create('QM' , isMinus , itDecision , self));
  e.Add(TIdentificator.Create('DM' , isPlus  , itDecision , self));
  e.Add(TIdentificator.Create('QJ' , isPlus  , itDecision , self));
  e.Add(TIdentificator.Create('SB' , isMinus , itConst    , self));
  FEQL.Add(e);
...
begin
  ...
  act := CreateAction(Actions, nil, 'Visualizar', false, nil, self);
         ...
         act := CreateAction(Actions, act, 'em Planilha', false, nil, self);
                ...
                // Demandas
                act := CreateAction(Actions, act, 'Demandas', false, nil, self);
...
begin
  ...
  if FDialogo <> nil then
     ...
     R3  := TprDialogo_PCPR(FDialogo).edVolMax.AsFloat;
...
begin
  ...
  // Equacao 0:  - SF + SI + PR - EV + QM - DM - QJ = - SB
  e := TEquation.Create();
  e.Add(TIdentificator.Create('SF' , isPlus  , itDecision  , self));
  e.Add(TIdentificator.Create('SI' , isMinus , itDecision  , self));
  e.Add(TIdentificator.Create('PR' , isMinus , itDecision  , self));
  e.Add(TIdentificator.Create('EV' , isPlus  , itDecision  , self));
  e.Add(TIdentificator.Create('QM' , isMinus , itDecision  , self));
  e.Add(TIdentificator.Create('DM' , isPlus  , itDecision  , self));
  e.Add(TIdentificator.Create('QJ' , isPlus  , itDecision  , self));
  e.Add(TIdentificator.Create('SB' , isMinus , itConst     , self));
...
begin
  // 0 = SI - SF + PR - EV - DM - QJ + QM + SB

  if FieldName = 'SI' then
     Result := 'STI_NAO_DEF'
  else
  if FieldName = 'SF' then
     Result := 'STF_NAO_DEF'
  else
  if FieldName = 'PR' then
     Result := FloatToStr( PrecipitacaoUnitaria[FProjeto.DeltaT] )
  else
  if FieldName = 'EV' then
     Result := FloatToStr( EvaporacaoUnitaria[FProjeto.DeltaT] )
  else
    Result := inherited io_getValue(FieldName);
end;
...
begin
  ...
  inherited getActions(Actions);
  CreateAction(Actions, FMenuAction_Sheet, 'Volume', false, ShowSheetEvent, self);
end;
...
function TprSubBacia.ObterVazaoAfluente(PC: TprPC): Real;
var i   : Integer;  // Intervalo de Simulação
    k   : Integer;  // Contador
    CC  : Real;     // Coef. Contribuição
    D   : Real;     // Somatório das demandas difusas
    DM  : TprDemanda; // n-égima Demanda
    no1 : TTreeNode; // Para debug
    no2 : TTreeNode; // Para debug
begin
  i := FProjeto.DeltaT;

  {$ifdef DEBUG}
  no1 := Applic.Debug.FindNode(FNome);
  if no1 = nil then no1 := Applic.Debug.Add(nil, FNome, FNome);
  no1 := Applic.Debug.Add(no1, 'DeltaT: ' + toString(i));
  {$endif}
  ...
  for k := 0 to Demandas-1 do
    if Demanda[k].Ligada then
       begin
       DM := Demanda[k];
       {$ifdef DEBUG} no2 := Applic.Debug.Add(no1, DM.Nome); {$endif}
       ...
       {$ifdef DEBUG}
       Applic.Debug.Add(no2, 'UnidadeDeConsumo: ' + toString(DM.UnidadeDeConsumo, 10));
       Applic.Debug.Add(no2, 'UnidadeDeDemanda: ' + toString(DM.UnidadeDeDemanda, 10));
       Applic.Debug.Add(no2, 'FatorDeConversao: ' + toString(DM.FatorDeConversao, 10));
       {$endif}
       end;

  Result := Max(0, (Vazoes[i] - D)) * CCs[PCs.IndiceDo(PC)];
  {$ifdef DEBUG}
  Applic.Debug.Add(no1, 'ObterVazaoAfluente(' + PC.Nome + '): ' + toString(Result, 10));
  {$endif}
end;
...
begin
  ...
  for i := 1 to ds.nRows do
    ...
    for k := 1 to 12 do
      TVU_Demanda(i-1, k, v[k] / DateUtils.DaysInAMonth(a, k) / 8.64);
    end;
end;
        ...
        try
          ...
          if (PE1 <> '') or (PE2 <> '') then
             ...
             s := 'A simulação não foi finalizada.'#13 +
                  '  - Intervalo: ' + toString(FDeltaT) + #13 +
                  '  - Erros:'#13;

             if PE1 <> '' then
                begin
                s := s + '    Ponto de Execução: ' + PE1 + #13 + Erro1;
                end;

             if PE2 <> '' then
                begin
                if PE1 <> '' then s := s + #13;
                s := s + '    Ponto de Execução: ' + PE2 + #13 + Erro2;
                end;
...
begin
  ...
  book.TextPage.Write(FloatToStr(NivelDeFalha));
...
function TprListaDeFalhas.Adicionar(Tipo: TEnumPriorDemanda; Intervalo: Integer;
...
begin
{
  if FEquacoes_PreVisualizar then
     GerarEquacoes_0(Text)
  else
     case FEquacoes_TipoGerador of
       tgAutomatico : GerarEquacoes_1(Text);
       tgManual     : GerarEquacoesPorScript(Text);
       end;
}
...
begin
  if FEquacoes_NomeScriptGeral <> '' then
     try
       ...
       s.Text.LoadFromFile(FEquacoes_NomeScriptGeral);
       s.Variables.AddVar(
...
// Palavras-Chaves: Equacao, Equacoes, Equation, Equations
procedure TprProjeto.Equacoes_GerarBalancoHidrico(DeltaT: integer);
var i: Integer;
begin
  FDeltaT := DeltaT;
  for i := 0 to PCs.PCs-1 do
    PCs[i].GerarEquacoes(FEQB.Text, DeltaT, FEquacoes_IndiceGeral);
end;
...
var
  ...
  begin
    ...
    sClass := no.attributes.item[0].nodeValue;
    sName := no.attributes.item[1].nodeValue;

    // Posicoes de tela (sx, sy)
    X := no.attributes.item[2].nodeValue;
    Y := no.attributes.item[3].nodeValue;

    // Posicoes reais (rx, ry)
    if no.attributes.length > 5 then
       begin
       Pos.X := no.attributes.item[5].nodeValue;
       Pos.Y := no.attributes.item[6].nodeValue;
       end
    else
       begin
       Pos.X := X;
    ...
    if sClass = TprCenarioDeDemanda.ClassName then
       begin
       sFactory := no.attributes.item[4].nodeValue;
  ...
  var HC1, HC2: THidroComponente;
      i: integer;
  begin
    i := Objects.IndexOf(no.attributes.item[0].nodeValue);
    HC1 := THidroComponente(Objects.Objects[i]);

    i := Objects.IndexOf(no.attributes.item[1].nodeValue);
...
begin
  for i := 0 to no.childNodes.length - 1 do
    ...
    Adicionar(child.attributes.item[0].nodeValue,
              child.attributes.item[1].nodeValue,
              child.attributes.item[2].nodeValue,
              child.attributes.item[3].nodeValue);                                                      
    end;
end;
...
begin
  if FOptimizer is TGeneticOptimizer then
     ...
     if FOptimizer = nil then
        ...
        raise Exception.Create('O otimizador ativo não é o "GeneticOptimizer"');
end;
...
{
  ihc_Simular();
  if ihc_TemErros() then
     begin
     Erros := TStringList.Create();
     ihc_ObterErros(Erros);
     s := Erros.Text;
     Erros.Free;
     MostrarErro(Format(cMsgErro09, [FNome, s]));
     end
  else
}
     begin

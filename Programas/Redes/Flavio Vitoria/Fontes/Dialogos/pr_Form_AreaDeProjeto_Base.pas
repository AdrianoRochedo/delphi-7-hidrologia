...
uses Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
     ActnList, Menus, ExtCtrls, FileCtrl, ComCtrls, StdCtrls,
     MSXML4,
     XML_Utils,
     pr_Application,
     pr_Classes,
...
type
  ...
  private
    ...
    procedure SetObjSel(const Value: TComponente);
    function ObterObjetoAssociado(Sender: TObject): TComponente;
  protected
    // Eventos
    ...
    procedure MostrarMenu(Obj: TComponente); // nao virtual
    ...
    function ObjetoPodeSerConectadoEm(const ID: String; Objeto: TComponente): Boolean;
    ...
    procedure ExecutarClick(Sender: TObject; Pos: TMapPoint; Obj: TComponente); virtual;
  ...
  public
    ...
    property ObjetoSelecionado : TComponente read FObjSel write SetObjSel;
...
implementation
uses WinUtils, GraphicUtils, Shapes, MessageManager,
     MAIN,
     Plugin,
     SysUtilsEx,
     pr_Funcoes,
...
// virtual
function TfoAreaDeProjeto_Base.CriarProjeto(TN: TStrings): TprProjeto;
begin
  NaoImplementado('CriarProjeto()');
end;

// public
procedure TfoAreaDeProjeto_Base.RemoverObjeto(Obj: TComponente);
begin
  if (Obj <> nil) and
     ...
     if Obj is TprPC then
        if FProjeto.PCs.Remover(TprPC(Obj)) then
...
// private
procedure TfoAreaDeProjeto_Base.SetObjSel(const Value: TComponente);
...
function TfoAreaDeProjeto_Base.ObjetoPodeSerConectadoEm(const ID: String; Objeto: TComponente): Boolean;
begin
  {TODO 1 -cProgramacao: Descarga: ObjetoPodeSerConectadoEm}
  Result := ((ID = 'Sub-Bacia') and (Objeto is TprPC)) or
            ((ID = 'QA') and (Objeto is TprPC) and (TprPC(Objeto).QualidadeDaAgua = nil)) or
            ((ID = 'Descarga') and (Objeto is TprPC)) or
            ((ID = 'Demanda') and (Objeto is TprPC));
end;

// Virtual
procedure TfoAreaDeProjeto_Base.ExecutarClick(Sender: TObject; Pos: TMapPoint; Obj: TComponente);
var Obj2: TComponente;
begin
  ...
  if FOnGetButtonStatus('Criar PC') or
     ...
     FProjeto.PCs.Adicionar(TprPC(Obj));
...
//---------------- Clicks em um PC de Controle -----------------------------------
  ...
  if FOnGetButtonStatus('Criar SB') and (Obj is TprPC) then
     begin
     Obj2 := Obj;
  ...
  if FOnGetButtonStatus('Criar Descarga') and (Obj is TprPC) then
     begin
     Obj2 := Obj;
...
(*
  if FOnGetButtonStatus('Criar Demanda') and
     ((Obj is TprPC) or (Obj is TprSubBacia)) then
     begin
     if ObjetoPodeSerConectadoEm('Demanda', Obj) then
        ...
        if ValidateDemand(Obj2) then
           ...
           if Obj is TprPC then
              TprDemanda(Obj2).Visivel := TprPC(Obj).MostrarDemandas;
...
*)
  if FOnGetButtonStatus('Criar TD') and (Obj is TprPC) then
     begin
     if FPC = nil then
        begin
        FPC := TprPC(Obj);
  ...
  if FOnGetButtonStatus('Inserir PC') and (Obj is TprPC) then
     begin
     if (FPC = nil) then
        begin
        FPC := TprPC(Obj);
...
// privado
function TfoAreaDeProjeto_Base.ObterObjetoAssociado(Sender: TObject): TComponente;
begin
  Result := nil;
  if (Sender is TdrBaseShape) or (Sender is TImage) then
     Result := TComponente(Classes.TComponent(Sender).Tag)
  else
...
procedure TfoAreaDeProjeto_Base.MostrarMenu(Obj: TComponente);
var Menu : TMenuItem;

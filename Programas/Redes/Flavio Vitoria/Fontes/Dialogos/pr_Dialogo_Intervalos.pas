...
begin
  ...
  s := IntervaloParaDataComoString(Fprojeto, ini) + ' - ' + IntervaloParaDataComoString(Fprojeto, Fim);
...
begin
  ...
  SubStrings('-', s1, s2, s);
  ini := strToInt(AllTrim(s1));
  fim := strToInt(AllTrim(s2));
  s   := SubString(lbInts.Items[i], '[', ']');
...
begin
  ...
  edDI.Text := IntervaloParaDataComoString(FProjeto, ini);
  edDF.Text := IntervaloParaDataComoString(FProjeto, fim);
...
begin
  ...
  SubStrings('/', s1, s2, edDF.Text);
  mes := strToInt(AllTrim(s1));
  Ano := strToInt(AllTrim(s2));
  ...
  if LI.Count > 0 then
     ...
     s := IntervaloParaDataComoString(Fprojeto, LI[0]) + ' - ' + IntervaloParaDataComoString(Fprojeto, LI[LI.Count-1]);
...
begin
  Dialogs.ShowMessage('Ainda não implementado');
end;

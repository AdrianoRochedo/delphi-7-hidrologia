...
!WC
c      Unidades: Cotas em M, VAZÃO EM M3/S E VOLUMES EM 1000 m3
c
      READ(1,4)NO,SI,zmax,npzs
...
c
      np(1)=npzs
      read(1,22)ivert,iorif,QLIM  !indicam vertedor, orificio e vazao MAXIMA do bypass
...
c
	...
	READ(1,3)Cvert,XL,Zvert !LÊ PARÂMETROS DO VERTEDOR
...
166	if(iorif.ne.0)READ(1,3)CORIF,AORIF,ZORIF
...
776   FORMAT('OPHID',i5,5I10)
	...
	aa=q(1,IS)+qDUT(1)
      WRITE(7,777)1,COTJ,Q(1,IS),QDUT(1),AA !escreve no aruivo
      DO 180 J=2,NT
	SOLD=ST
	QI2=QM(J)
	NPP=NP(IO)
...
21	VAL=(QI2+QI1)/2.-QE/2+ST
	...
	WRITE(7,777)J,COTJ,Q(J,IS),QDUT(J),AA
...
1	FORMAT(20A4)
2	FORMAT(8I10)
22    FORMAT(2I10,F10.2)
3	FORMAT(8F10.2)
4	FORMAT(I10,2F10.0,I10)

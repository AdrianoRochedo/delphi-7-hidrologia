      ...
      SUBROUTINE AMCPFECH(NP)   !planicie de inundacao em condutos fechados
	USE VAR  
	use varexcesso

      DIMENSION H(30),qt(30),cel(31),qfulls(10)
     1,ak(30),xxx(30),sar(50),ATOT(30),A(30,10),Qmaximas(10),
     2itipe(30),B(30),y0(30),Qx(30,10),Tx(30,10),t(30)

	real, allocatable:: stor(:,:)
      real cotmon,cotjus
	integer secion
	integer ntrech
	INTEGER FLAG1
	integer ANCHO,ALTO,ZESQ,ZDIR 
      real cele
      real along
...
!     A PROGRAMACAO E CONFUSA MAS FUNCIONA (daniel)
	...
	READ(1,'(F10.2,I10)')QREF,excesso 

      select case (excesso) !le a linha seguinte dependendo da escolha de que fazer comk excessos
		case (1)
		read (1,'(8i10)')secion,ANCHO,ALTO,ZESQ,ZDIR     !le em que dimensões amplia, caso seja tubo deixar linha em branco
		case (2)
		read (1,'(f10.3)')rugrua
	end select
...
500   IF (NP.EQ.0)nP=1 !por default num trechos paralelos= 1
      ...
      READ(1,'(I10,9F10.2,2I10)')itipe(j),y0(J),B(J),
	1zl(j),zr(j),RUG(j),ALONG,COTMON,
     *COTJUS,DTCAL,NTRECH,ILAT 
	xlong=along
	SO=(COTMON-COTJUS)/ALONG !calcula a declividade
	 ...
	 dO 501 J=2,NP
	 READ(1,'(I10,8F10.2,2I10)')itipe(j),y0(J),B(J),zl(j),zr(j),RUG(j)
501	 continue
	endif
...
c     ............................................................
			if (H(i).LE.y0(j)) then
			QX(I,J)=(B(j)*h(i))**(5./3.)/RUG(J)/
	1		(B(j)+2*h(i))**.66667*SO**.5 !VAZAO NA FAtiA
...
c     ............................................................
		...
		Pmmx=0.5*(b(j)+2.*ys*sqrt(1+zl(j)*zl(j)))+
     1	0.5*(b(j)+2.*ys*sqrt(1+zr(j)*zr(j)))
		Rmmx=A(I,J)/Pmmx
...
503			continue
	...
	!revise y avise si tiene error
			...
			if(np==1.AND.itipe(1)==2.AND.h(i)>0.94*y0(1))
	1		cel(i)=cel(i-1)
...
102      IF (.NOT. ALLOCATED(qm))ALLOCATE (qm(ncalc))
         DO 202 K=1,NT
202     QM(K)=Q(K,IE)
	...
	IF (.NOT. ALLOCATED(ql))ALLOCATE (QL(ncalc))
      ...
      IF (.NOT. ALLOCATED(qexc))ALLOCATE (qexc(NTRECH+1,ncalc))
...
c	- ------------EXCESSOS ----------
	...
	if(excesso==2)write(6,'(i10,F10.2)')I,Qexceso(I)

	if(.not.allocated(stor))allocate(stor(nt,ns))
...
1040  continue
	...
	IF (FLAG1==0)THEN !SAIDA DO DIMENSIONAMENTO
      FLAG1=1
      write(9,*)'OPERAÇAO ',ca
	Write(9,*)'SECAÓ',NP,'NOVO DIAMETRO',Y0(J)
      ELSE
	BACKSPACE(9)
      Write(9,*)'SECAÓ',NP,'NOVO DIAMETRO',Y0(J)
	ENDIF !FIM SAIDA DIMENSIONAMENTO
	...
	IF (FLAG1==0)THEN !SAIDA DO DIMENSIONAMENTO
      FLAG1=1
      write(9,*)'OPERAÇAO ',ca
	Write(9,*)'SECAÓ',NP,'REDIMENSIONADA PARA'
      Write(9,*)'ALTURA',Y0(J),'LARGURA',B(J)
      ELSE
	BACKSPACE(9)
      Write(9,*)'ALTURA',Y0(J),'LARGURA',B(J)
	ENDIF !FIM SAIDA DIMENSIONAMENTO
	...
	IF (FLAG1==0)THEN !SAIDA DO DIMENSIONAMENTO
      ...
      write(9,*)'OPERAÇAO ',ca
	Write(9,*)'SECAÓ',NP,'REDIMENSIONADA PARA'
      Write(9,334)Y0(J),B(J),ZL(J),ZR(J)
      ELSE
	BACKSPACE(9)
      Write(9,334)Y0(J),B(J),ZL(J),ZR(J)
	ENDIF !FIM SAIDA DIMENSIONAMENTO
...
!  WRITE(2,*)'O TRECHO ALAGOU-VERIFIQUE .ALG'
	  write(16,5986)
	  write(16,'(i10,4f10.3)')is,alaga,duracao
	endif



	deALLOCATE (QL,sto,qc,QM,QCOR,qexc,stor) 
...
334   format('ALTURA',f8.2,'  LARGURA',f8.2,'  Zesq',f8.2,'  Zdir',f8.2)

unit pr_Dialogo_Planilha_DemandasDeUmaClasse;
...
uses
  ...
  ExtCtrls, pr_Classes, pr_AreaDeProjeto, Mask, drEdit;

type
  TprDialogo_Planilha_DemandasDeUmaClasse = class(TprDialogo_PlanilhaBase)
    Panel1: TPanel;
  ...
  public
    ...
    property AreaDeProjeto : TprDialogo_AreaDeProjeto read FAP write FAP;
  end;
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.SetPeriodo(const Value: Integer);
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.FormShow(Sender: TObject);
...
begin
  ...
  cbPeriodos.Clear;
  CD := TprClasseDemanda(ObterObjetoPeloNome(FClasse.Nome, FAP.Projeto));
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.cbPeriodosChange(Sender: TObject);
begin
  inherited;
  Periodo := cbPeriodos.ItemIndex;
end;

procedure TprDialogo_Planilha_DemandasDeUmaClasse.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  RecuperaValores;
  Action := caFree;
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.TabSelChange(Sender: TObject);
var R, C, ii, Mes : Integer;
    DM            : TprDemanda;
begin
  inherited;
  Tab.GetActiveCell(R, C);
  DM := TprDemanda(ObterObjetoPeloNome(Tab.TextRC[R, 2], FAP));
  if DM <> nil then
     for ii := 1 to FAP.Projeto.TotalAnual_IntSim do
       begin
       FAP.Projeto.DeltaT := ii;
       Mes := FAP.Projeto.Data.Mes;
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.TabEndEdit(
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.FormCreate(Sender: TObject);
begin
  inherited;
  FPriVez := True;
  Tab.FixedCols := 3;
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.CalcularDemanda(DM: TprDemanda; Linha: Integer);
var ii, Mes: Integer;
begin
  for ii := 1 to FAP.Projeto.TotalAnual_IntSim do
    begin
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.CalculaSomatorios;
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.edEscalaExit(Sender: TObject);
begin
  inherited;
  Try
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.edEscalaKeyDown(
...
procedure TprDialogo_Planilha_DemandasDeUmaClasse.RecuperaValores;
var i, j: Integer;
    SL  : TStrings;   // auxiliar
    DM  : TprDemanda; // Demanda atual
begin

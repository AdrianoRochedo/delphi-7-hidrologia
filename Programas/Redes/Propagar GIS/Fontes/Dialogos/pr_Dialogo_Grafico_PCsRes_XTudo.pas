...
begin
  ...
  if cbLinhas.Checked then
     ...
     if cbVol.Checked then
        begin
        b := g1.Series.AdicionaSerieDeLinhas('Volume', $00C7CBEF, TprPCPR(PC).Volume, Ini, Fim);
  ...
  if cbBarras.Checked then
     ...
     if cbVol.Checked then
        begin
        b := g2.Series.AdicionaSerieDeBarras('Volume', $00C7CBEF, 0, 1, TprPCPR(PC).Volume, Ini, Fim);
        PC.DefinirEixoX_Default(b, k);
        end;
     end;
  ...
  if cbPontos.Checked then
     ...
     if cbVol.Checked then
        begin
        b := g1.Series.AdicionaSerieDePontos('Volume', $00C7CBEF, TprPCPR(PC).Volume, Ini, Fim);
        PC.DefinirEixoX_Default(b, k);
        if cbFalhaVolume.Checked then
           for i := Ini to Fim do
              if TprPCPR(PC).Volume[i] < 0.6 * TprPCPR(PC).VolumeMaximo then

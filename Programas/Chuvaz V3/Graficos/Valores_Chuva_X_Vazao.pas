...
begin
  ...
  L_Delta.Caption := IntToStr(Delta);
  ...
  for i := 0 to FPostosDeChuva.Count - 1 do
    begin
    Canvas.TextOut(FMaxXc, y-3, Dados.AsString[Delta, ind_PostosDeChuva[i]]);
  ...
  for i := 0 to FPostosDeVazao.Count - 1 do
    begin
    Canvas.TextOut(FMaxXv, y-3, Dados.AsString[Delta, ind_PostosDeVazao[i]]);
    inc(y, AlturaX + OffsetY);
    end;
end;

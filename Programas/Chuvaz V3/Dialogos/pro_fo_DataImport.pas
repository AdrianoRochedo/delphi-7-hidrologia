...
uses
  ...
  Dialogs, WinUtils, ch_Tipos, pro_Interfaces, pro_Plugins;
...
begin
  if FActivePlugin.Load() then
     begin
     //FActivePlugin.setDBControlInterface(FDB);
...
begin
  ...
  for i := 0 to SL1.Count-1 do
    ...
    if (Length(v) > 0) and (v[1] = '(') then
       ...
       StringToStrings(SubString(v, '(', ')'), SL2, ['|']);
  ...
  try
    if FActivePlugin.Handle <> 0 then
       ...
       for i := 0 to lbArquivos.Count-1 do
         begin
         lbArquivos.ItemIndex := i;
         ipc_ShowMessage('Importando <' + lbArquivos.Items[i] + '>');
         try
           if FActivePlugin.Import(lbArquivos.Items[i]) then
              ipc_ShowMessage('Ok.');
         except
           // nada
         end;
